...
/// Options for capturing a persistent snapshot from a running sandbox.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct SnapshotCaptureOptions {
    /// Capture disk state only (rootfs and attached drives), skipping the
    /// Firecracker VM state and memory snapshot. Disk-only snapshots are much
    /// cheaper to store but cannot be resumed: sandboxes created from them
    /// boot fresh from the captured rootfs.
    pub disk_only: bool,
}
