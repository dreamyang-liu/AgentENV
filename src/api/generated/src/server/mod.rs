    ...
    C: Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        .route(
            "/snapshots/{snapshot_id}/squash",
            post(snapshots_snapshot_id_squash_post::<I, A, E, C>),
        )
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::snapshots::SnapshotsSnapshotIdDeleteResponse::Status204_SnapshotDeleted => {
                let mut response = response.status(204);
                response.body(Body::empty())
            }
            ...
            apis::snapshots::SnapshotsSnapshotIdDeleteResponse::Status409_TheIdNamesATemplate(
                body,
            ) => {
                let mut response = response.status(409);
                {
            ...
            apis::snapshots::SnapshotsSnapshotIdDeleteResponse::Status500_ServerError(body) => {
                let mut response = response.status(500);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers
...
#[derive(validator::Validate)]
#[allow(dead_code)]
struct SnapshotsSnapshotIdSquashPostBodyValidator<'a> {
    #[validate(nested)]
    body: &'a models::SnapshotSquashRequest,
}

#[tracing::instrument(skip_all)]
fn snapshots_snapshot_id_squash_post_validation(
    path_params: models::SnapshotsSnapshotIdSquashPostPathParams,
    body: models::SnapshotSquashRequest,
) -> std::result::Result<
    (
        models::SnapshotsSnapshotIdSquashPostPathParams,
        models::SnapshotSquashRequest,
    ),
    ValidationErrors,
> {
    path_params.validate()?;
    let b = SnapshotsSnapshotIdSquashPostBodyValidator { body: &body };
    b.validate()?;
...
#[tracing::instrument(skip_all)]
async fn snapshots_snapshot_id_squash_post<I, A, E, C>(
    method: Method,
    TypedHeader(host): TypedHeader<Host>,
    cookies: CookieJar,
    headers: HeaderMap,
    Path(path_params): Path<models::SnapshotsSnapshotIdSquashPostPathParams>,
    State(api_impl): State<I>,
    Json(body): Json<models::SnapshotSquashRequest>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
    A: apis::snapshots::Snapshots<E, Claims = C>
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        snapshots_snapshot_id_squash_post_validation(path_params, body)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .snapshots_snapshot_id_squash_post(&method, &host, &cookies, &claims, &path_params, &body)
        .await;
    ...
    let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::snapshots::SnapshotsSnapshotIdSquashPostResponse::Status201_SquashedSnapshotPublished
                                                    (body)
                                                => {
                                                  let mut response = response.status(201);
                                                  {
                                                ...
                                                apis::snapshots::SnapshotsSnapshotIdSquashPostResponse::Status400_BadRequest
                                                    (body)
                                                => {
                                                  let mut response = response.status(400);
                                                  {
                                                ...
                                                apis::snapshots::SnapshotsSnapshotIdSquashPostResponse::Status401_AuthenticationError
                                                    (body)
                                                => {
                                                  let mut response = response.status(401);
                                                  {
                                                ...
                                                apis::snapshots::SnapshotsSnapshotIdSquashPostResponse::Status404_NotFound
                                                    (body)
                                                => {
                                                  let mut response = response.status(404);
                                                  {
                                                ...
                                                apis::snapshots::SnapshotsSnapshotIdSquashPostResponse::Status500_ServerError
                                                    (body)
                                                => {
                                                  let mut response = response.status(500);
                                                  {
